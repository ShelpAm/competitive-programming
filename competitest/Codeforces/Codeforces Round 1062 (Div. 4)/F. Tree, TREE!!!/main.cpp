...
// Start: Wed 29 Oct 2025 02:04:55 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    int n, k;
    std::cin >> n >> k;
    auto g = graph::Graph::read(n + 1, n - 1, false, false);

    std::vector<int> sz(n);
    std::vector<int> ok(n);   // num of sons that're ok
    std::vector<int> isok(n); // num of sons that're ok
    auto dfs = [&](auto dfs, int u, int p) -> void {
        sz[u] = 1;
    ...
    i64 ans{};
    // oku: num of ok of u tree
    auto run = [&](auto run, int u, int p, int oku) -> void {
        debug("oku", oku);
        ans += oku;
        for (auto [_, v] : g.edges_of(u)) {
            if (v != p) {
                debug("v", v);
                debug("?", 1 + oku - isok[v]);
                run(run, v, u, (n - sz[v] >= k) + oku - isok[v]);
            }
        }
    };
    run(run, 0, 0, ok[0] + isok[0]);
    std::cout << ans << '\n';
}
