...
// Start: Tue 30 Dec 2025 04:35:29 PM CST
// Author: ShelpAm
...
auto fact(auto t, int k = 1)
{
    i64 res{1};
    for (auto e : std::views::iota(k, t + 1))
        res = res * e % mod998244353;
    debug(std::format("fact({}) / fact({})", t, k - 1), res);
    return res;
};
void solve_case()
{
    ...
    auto [min, max] = std::ranges::minmax(a);
    int diff{};
    for (auto e : a) {
        diff += std::max(0, max - 1 - e);
    }
    if (diff > c) {
        std::cout << 0 << '\n';
        return;
    ...
    auto ex = c - diff;
    auto t = std::ranges::count(a, max);
    debug("ex,t", std::pair{ex, t});
    if (ex + t >= n)
        std::cout << fact(n) << '\n';
    else {
        std::cout << fact(n - t) * fact(t + ex, ex + 1) % mod998244353 << '\n';
    }
}
