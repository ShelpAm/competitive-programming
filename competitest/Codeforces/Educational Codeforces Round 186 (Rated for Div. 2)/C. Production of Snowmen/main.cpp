...
// Start: Tue 30 Dec 2025 04:12:13 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    int n;
    std::cin >> n;
    std::vector a(3, std::vector<i64>(n));
    std::cin >> a;
    ...
    for (auto const &r : a | std::views::slide(2)) {
        ...
        auto y = r[1];
        int t{};
        for (int off{}; off != n; ++off) {
            if (std::ranges::all_of(std::views::iota(0, n), [&](auto i) {
                    return x[i] < y[(i + off) % n];
                })) {
                t += 1;
            }
        }
