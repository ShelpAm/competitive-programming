...
// https://codeforces.com/gym/104857/problem/J Start: Sun 05 Oct 2025 01:35:42
    ...
    -> Dijkstra_result
{
    ...
    while (!q.empty()) { // The main loop
        ...
        for (auto const &[w, v] : g.edges_of(u)) {
            if (auto const alt{std::max(dist[u], w)}; dist[v] > alt) {
                dist[v] = alt;
                prev[v] = u;
                q.push({alt, v});
            }
...
void solve_case()
{
    ...
    int n, m;
    std::cin >> n >> m;
    auto g = graph::Graph::read(n, m, false, true);

    auto d1 = graph::dijkstra(g, 0).distance;
    auto d2 = graph::dijkstra(g, n - 1).distance;
