...
// https://codeforces.com/gym/105486/problem/J Start: Thu 23 Oct 2025 02:33:54
...
void solve_case()
{
    ...
    std::cin >> n >> m >> q;
    int active{-1};
    std::vector<std::set<int>> ok(n);
    std::vector<std::set<int>> giveup(n);
    std::vector<std::pair<i64, int>> score(m);
    ...
    for (int i{}; i != q; ++i) {
        ...
        if (t == 1) {
            ...
            if (ok[active].contains(id) || giveup[active].contains(id)) {
                continue;
            }
            score[id].first += m - ok[active].size();
            ok[active].insert(id);
        }
        else if (t == 3) {
            int id, x;
            ...
            giveup[active].insert(id);
        }
    }
    std::ranges::sort(score | std::views::reverse);
    for (auto [x, id] : score) {
        std::cout << -id + 1 << ' ' << x << '\n';
    }
}
