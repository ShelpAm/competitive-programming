...
// Start: Wed 08 Oct 2025 06:30:03 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    std::vector f(n + 1, std::vector<int>((n * n) + 1, -1));
    f[0][0] = 0;
    for (int i{1}; i != n + 1; ++i) {
        for (int k{}; k != f[i].size(); ++k) {
            for (int j{1}; i - j >= 0 && k - ((j - 1) * j / 2) >= 0; ++j) {
                if (f[i - j][k - ((j - 1) * j / 2)] != -1) {
                    f[i][k] = j;
                }
            }
        }
    ...
    for (int i{1}; i != n + 1;) {
        ...
        std::reverse(seq.begin() + i, seq.begin() + m + 1);
        i = m + 1;
    }

    for (auto e : seq | std::views::drop(1)) {
        std::cout << e << ' ';
    }
    std::cout << '\n';
}
