...
// Start: Sat 04 Oct 2025 12:09:39 AM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    std::unordered_map<int, int> f_;
    f_[1] = 0;
    std::function<int(int)> f = [&](int x) -> int {
        if (f_.contains(x)) {
            return f_[x];
        }
        return f_[x] = f((x / 2) + (x / 2 == 1 ? 0 : 1)) + 1;
    };

    // for (int i{1}; i != 20; ++i)
    //     debug("f " + std::to_string(i), f(i));
    ...
    for (int i{}; i != q; ++i) {
        int l, r;
        std::cin >> l >> r;
        std::cout << p[r] - p[l - 1] + ((d[r] - d[l - 1]) / 2) << '\n';
    }
}
