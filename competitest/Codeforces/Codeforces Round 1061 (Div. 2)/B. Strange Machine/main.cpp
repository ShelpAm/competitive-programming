...
// Start: Sat 25 Oct 2025 11:40:05 AM CST
// Author: ShelpAm
...
namespace {
[[maybe_unused]] constexpr std::int_least64_t mod998244353{998'244'353ULL};
[[maybe_unused]] constexpr std::int_least64_t mod1e9p7{1'000'000'007ULL};
...
void solve_case()
{
    ...
    for (int i{}; i != q; ++i) {
        ...
        int ans{};
        if (has_div) {
            for (int j{}; x != 0; ++j %= a.size()) {
                if (a[j].first == 'A') {
                    auto t = std::min(a[j].second, x);
                    ans += t;
                    x -= t;
                }
                else {
                    auto t =
                        std::min(a[j].second, msb(static_cast<u64>(x)) + 1);
                    ans += t;
                    x >>= t;
                }
            }
