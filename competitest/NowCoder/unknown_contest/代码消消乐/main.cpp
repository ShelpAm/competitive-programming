...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/118113/J
// Start: Sat 27 Sep 2025 02:56:25 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    auto solve = [](std::string const &s) {
        std::list<char> l(s.begin(), s.end());
        for (auto it = std::next(l.begin()); it != l.end(); ++it) {
            while (it != l.end() && it != l.begin() && *it == *std::prev(it)) {
                it = l.erase(std::prev(it));
                it = l.erase(it);
            }
            debug("l", l);
        }
