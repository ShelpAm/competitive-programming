...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/118113/H
// Start: Sat 27 Sep 2025 02:09:22 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    std::vector<std::array<int, 1 << 3>> f(n + 1);
    for (int i{1}; i != n + 1; ++i) {
        f[i][0] = std::ranges::max(f[i - 1]);
        for (unsigned mask{}; mask != 1 << 3; ++mask) {
            ...
            if (std::ranges::any_of(
                    v, [&](auto v) { return a[v][i - 1] == '0'; })) {
                continue;
            }
            for (unsigned prev{}; prev != 1 << 3; ++prev) {
                if (prev != mask) {
                    chmax(f[i][mask], f[i - 1][prev] + 1);
                }
            }
        }
    }
    debug("f", f);
    std::cout << std::ranges::max(f[n]) << '\n';
}
