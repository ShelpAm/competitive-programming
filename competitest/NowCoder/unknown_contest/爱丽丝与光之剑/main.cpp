...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/118113/K
// Start: Sat 27 Sep 2025 03:42:59 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    double d = std::abs((x1 * y2) - (x2 * y1)) /
               std::sqrt(std::pow(y1 - y2, 2) + std::pow(x1 - x2, 2));
    ...
    auto alpha = std::acos(std::min(1., d / a));
    double ss = a * a * alpha;                    // 扇形
    double st = d * std::sqrt((a * a) - (d * d)); // 三角形

    double lie = ss - st;
    double ans = lie / (3.14159265 * a * a - lie);
    std::cout << ans << '\n';
}
