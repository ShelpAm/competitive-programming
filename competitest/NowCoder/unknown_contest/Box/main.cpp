...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/117511/K
// Start: Wed 08 Oct 2025 12:08:06 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    std::vector f(n + 1, std::vector(4, -inf<i64>)); // f i j:
    // 前i个，且第i个状态为j
    // f[i]:
    //  0: 无
    //  1: 有，不可移动
    ...
    for (int i{1}; i != n + 1; ++i) {
        if (b[i]) {
            f[i][0] = f[i - 1][0] + a[i - 1];            // move back
            f[i][2] = std::ranges::max(f[i - 1]) + a[i]; // stay as is
            f[i][3] =
                std::max({f[i - 1][3] + a[i],
                          f[i - 1][2] - a[i - 1] + a[i]}); // keep more than one
        }
        else {
            f[i][0] = std::ranges::max(f[i - 1]);    // stay as is
            f[i][1] = f[i - 1][2] - a[i - 1] + a[i]; // move from back
            chmax(f[i][1], f[i - 1][3] + a[i]);      // move from back(3)
        }

        for (int j{}; j != 4; ++j)
            std::cerr << std::format("f({},{})={}\n", i, j, f[i][j]);
    }

    i64 ans{};
    for (auto const &e : f[n]) {
        chmax(ans, e);
    }
