...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/118113/C
// Start: Sat 27 Sep 2025 01:58:06 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    std::vector<i64> maxf(4);
    std::vector<i64> minf(4);
    for (auto [x, y] : a) {
        i64 ans{};
        std::vector<int> f{x + y, x - y, -x + y, -x - y};
        for (int i{}; i != 4; ++i) {
            chmin(minf[i], f[i]);
            chmax(maxf[i], f[i]);
            chmax(ans, maxf[i] - minf[i]);
        }
        std::cout << ans << '\n';
    }
}
