...
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/117511/F
// Start: Wed 08 Oct 2025 01:03:20 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    for (int n = 2; n != 5; ++n) {
        auto conv = [&](int i, int j, int k) { return k + n * (j + n * i); };
        std::cout << n << '\n';
        for (int r{}; r != n; ++r) {
            for (int g{}; g != n; ++g) {
                for (int b{}; b != n; ++b) {
                    std::vector<int> vis(n * n * n);
                    auto d = std::views::iota(-1, 2);
                    std::map<std::pair<std::vector<int>, std::array<int, 3>>,
                             bool>
                        memo;
                    std::function<bool(int, int, int)> dfs = [&](int x, int y,
                                                                 int z) {
                        auto state = std::pair{vis, std::array{x, y, z}};
