...
// Problem: 哦！昨日重重重重重重重现！
// Contest: unknown_contest
// Judge: NowCoder
// URL: https://ac.nowcoder.com/acm/contest/118113/A
// Start: Sat 27 Sep 2025 01:25:40 PM CST
// Author: ShelpAm
...
void solve_case()
{
    ...
    std::vector<std::pair<int, int>> dd{{0, 1}, {0, -1}, {-1, 0}, {1, 0}};

    auto once = [&](int u, int v) {
        auto r = t[{u, v}];
        for (int j{}; j != 4; ++j) {
            if (dirs[r] & 1 << j) {
                for (int k{}; k != 4; ++k) {
    ...
    bool const onedir{c['U'] == 0 && c['D'] == 0 || c['L'] == 0 && c['R'] == 0};
    for (int i{}; i != q; ++i) {
        int u, v;
        std::cin >> u >> v;
        if (!t.contains({u, v})) {
