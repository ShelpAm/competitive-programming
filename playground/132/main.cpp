...
void solve_case()
{
    ...
    std::multiset<int> rs(r.begin(), r.end());
    auto check = [&](int s1) -> std::pair<bool, std::vector<int>> {
        auto ms = rs;
        std::vector<int> ans{s1};
        for (int i{1}; i != n; ++i) {
            auto si = t[i] - ans.back();
            if (!ms.extract(si)) {
                return {false, {}};
            }
            ans.push_back(si);
    ...
    debug("s1o", s1o);
    debug("s1e", s1e);
    if (auto [ok, ans] = check(s1o); ok) {
        for (auto e : ans) {
            std::cout << e << ' ';
        }
        return;
    }
    if (auto [ok, ans] = check(s1e); ok) {
        for (auto e : ans) {
            std::cout << e << ' ';
        }
    }
