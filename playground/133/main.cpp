...
void solve_case()
{
    ...
    std::vector f(n + 1, std::vector<i64>(1 << 3, inf<i64>));
    f[0][0b111] = 0;
    for (int i{1}; i != n + 1; ++i) {
        for (int mask{}; mask != 1 << 3; ++mask) {
            if (mask >> 2 & 1) {
                chmin(f[i][mask << 1 & 0b111], f[i - 1][mask]);
            }
        }
        for (int j{1}; j != 4; ++j) {
            if (i - j <= 0) {
            ...
            if ((a[i].second ^ a[i - j].second) == 1) { // In original pairs
                continue;
            }
            for (int mask{}; mask != 1 << 3; ++mask) {
                if (mask << 1 >> j & 1) { // Already in use
                    continue;
                }
                if (!((mask << 1 | 1 << j) >> 3 & 1)) { // i-3 not used.
                    continue;
                }
                chmin(f[i][(mask << 1 | 1 | 1 << j) & 0b111],
                      f[i - 1][mask] + std::abs(a[i].first - a[i - j].first));
            }
        }
    }
